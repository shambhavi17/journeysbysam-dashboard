...
function renderCaption(){
  ...
  $('#psec-caption').innerHTML=`
    ...
    <div class="pcard pai-card">
      <textarea class="pnote-ed" id="ai-brief" rows="3" placeholder="Describe your post — where you were, what happened, the vibe, a detail to include..."></textarea>
      ...
      <div id="ai-cfg" style="display:none;margin-top:12px">
        ...
        <input type="text" class="pinput" id="ai-model" placeholder="claude-3-5-haiku-latest">
        <button class="pbtn pbtn-accent" id="ai-save" style="margin-top:10px">Save settings</button>
        <div class="psecdesc" style="margin-top:8px">Get a key at <b>console.anthropic.com &rarr; API Keys</b>. Needs a small prepaid balance (about 1&cent; per caption). If a model name errors, try <b>claude-3-5-haiku-latest</b> (cheapest) or <b>claude-3-5-sonnet-latest</b> (best).</div>
      </div>
    </div>
    <div class="pblock-title">&#9889; Quick Template Builder</div>
    <div class="pcard">
      <div class="pgrid pg2">
  ...
  $('#ai-settings').addEventListener('click',()=>{
    ...
    if(open){const c=aiCfg();$('#ai-key').value=c.key||'';
      $('#ai-model').value=c.model||'claude-3-5-haiku-latest';}
  });
  $('#ai-save').addEventListener('click',()=>{
    localStorage.setItem(AI_CFG,JSON.stringify({key:$('#ai-key').value.trim(),
      model:$('#ai-model').value.trim()||'claude-3-5-haiku-latest'}));
  ...
  $('#ai-gen').addEventListener('click',()=>{
    ...
    if(!c.key){$('#ai-status').textContent='⚠ add your API key in ⚙ AI settings first';
      ...
      const cc=aiCfg();$('#ai-key').value=cc.key||'';
      $('#ai-model').value=cc.model||'claude-3-5-haiku-latest';return;}
    const d=dests.find(x=>x.id===$('#cap-dest').value)||dests[0];
