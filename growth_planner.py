...
#pane-planner .pelrst{margin-top:8px;margin-left:8px;font-family:var(--p-sans);font-size:12px;
  font-weight:500;cursor:pointer;border:1px solid var(--p-line);background:transparent;
  color:var(--p-muted);border-radius:9px;padding:6px 12px;transition:.13s;}
#pane-planner .pelrst:hover{color:var(--p-accent-d);border-color:var(--p-accent);}
...
const EL_ORIG={};   /* scope -> original default list, for Restore */
...
function elDel(scope){const s=_ed();delete s[scope];delete s[scope+'::chk'];LS.set(K_EDIT,s);}
...
function elRstBtn(scope){
  return _ed()[scope]!=null
    ?`<button class="pelrst" data-elrst title="Reset this list to its original items">&#8635; Restore original</button>`:'';
}
function elList(scope,items,opts){
  opts=opts||{};const ml=opts.multiline?1:0;
  if(!(scope in EL_ORIG))EL_ORIG[scope]=(items||[]).slice();
  ...
  return `<div class="pellist" data-elscope="${esc(scope)}" data-elml="${ml}" data-ellabel="${esc(opts.label||'item')}">`+
    ...
    `<button class="paddbtn" data-eladd>+ Add ${esc(opts.label||'item')}</button>${elRstBtn(scope)}</div>`;
}
...
function wirePellist(box){
  ...
  box.querySelectorAll('[data-eldel]').forEach((b,i)=>b.addEventListener('click',()=>{
    const arr=pellistVals(box);
    if(!confirm('Delete this item?'+(arr[i]?'\n\n“'+arr[i]+'”':'')))return;
    arr.splice(i,1);rebuild(arr);}));
  ...
  const rst=box.querySelector('[data-elrst]');
  if(rst)rst.addEventListener('click',()=>{
    if(!confirm('Restore this list to its original items? Any changes you made to this list will be replaced.'))return;
    elDel(scope);
    const t=document.createElement('div');
    t.innerHTML=elList(scope,(EL_ORIG[scope]||[]).slice(),{multiline:box.dataset.elml==='1',label:box.dataset.ellabel});
    const fresh=t.firstElementChild;box.replaceWith(fresh);wirePellist(fresh);});
}
...
function wirePchecklist(box){
  ...
  box.querySelectorAll('[data-eldel]').forEach((b,i)=>b.addEventListener('click',()=>{
    const arr=pchkVals(box),chk=pchkStates(box);
    if(!confirm('Delete this item?'+(arr[i]?'\n\n“'+arr[i]+'”':'')))return;
    arr.splice(i,1);chk.splice(i,1);rebuild(arr,chk);}));
  const add=box.querySelector('[data-eladd]');
  if(add)add.addEventListener('click',()=>{
  ...
  if(rst)rst.addEventListener('click',()=>{
    ...
    elDel(scope);
    const t=document.createElement('div');
    t.innerHTML=elCheck(scope,(EL_ORIG[scope]||[]).slice(),{multiline:box.dataset.elml==='1',label:box.dataset.ellabel});
    const fresh=t.firstElementChild;box.replaceWith(fresh);wirePchecklist(fresh);});
}
...
function renderGrowth(){
  ...
  $('#psec-growth').querySelectorAll('[data-gcmdel]').forEach(b=>b.addEventListener('click',()=>{
    if(!confirm('Delete this month row?'))return;
